...
// Function to dynamically create participant buttons
function createParticipantButtons() {
  ...
  participants.forEach((participant, index) => {
    ...
    button.addEventListener("click", () => navigateToParticipantPage(index));
    participantsList.appendChild(button);
  });
}

// Function to navigate to the participant page
function navigateToParticipantPage(participantIndex) {
  const participant = participants[participantIndex];
  // For this basic template, we'll navigate to a separate page using the participant's name in the URL.
  // In a real-world scenario, you might use a router or more advanced navigation techniques.
  window.location.href = `participant.html?name=${encodeURIComponent(participant.name)}`;
}

// Function to update overall data
function updateOverallData() {
  const totalParticipantsElement = document.getElementById("totalParticipants");
  // Update other overall data elements if needed

  totalParticipantsElement.textContent = participants.length; // Update to the current number of participants
}

// JavaScript Code
const targetParticipants = 20;
const targetIncomeGrowth = 23926;
const targetWageIncrease = 2393;
const targetJobsFound = 13;

let currentParticipants = 0;
let currentIncomeGrowth = 0;
let currentWageIncrease = 0;
let currentJobsFound = 0;

const incrementValues = () => {
  // Increment the current values until they reach the target values
  if (currentParticipants < targetParticipants) {
    currentParticipants++;
    document.getElementById('totalParticipants').textContent = currentParticipants;
  }

  if (currentIncomeGrowth < targetIncomeGrowth) {
    currentIncomeGrowth += 100;
    document.getElementById('incomeGrowth').textContent = currentIncomeGrowth;
  }

  if (currentWageIncrease < targetWageIncrease) {
    currentWageIncrease += 10;
    document.getElementById('wageIncrease').textContent = currentWageIncrease;
  }

  if (currentJobsFound < targetJobsFound) {
    currentJobsFound++;
    document.getElementById('jobsFound').textContent = currentJobsFound;
  }

  // Check if all numbers have reached their targets
  if (
    currentParticipants >= targetParticipants &&
    currentIncomeGrowth >= targetIncomeGrowth &&
    currentWageIncrease >= targetWageIncrease &&
    currentJobsFound >= targetJobsFound
  ) {
...
const interval = setInterval(incrementValues, 50); // Adjust the interval (milliseconds) for smoother animation
